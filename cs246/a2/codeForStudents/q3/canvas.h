...
static constexpr int DEFAULT_RECT_SIZE = 10;
...
struct Canvas {
  ...
  //    Canvas dimensions may have been altered as a result
  void scale(int rectangleId, float heightFactor = 1.0,
...
private: // Everything that follows cannot be directly accessed.
  ...
  void resizeRectangles(int newCapacity) {
    Rectangle newRects[newCapacity];
    for (int i = 0; i < n_Rect; i++) {
      newRects[i] = std::move(rectangles[i]);
    }
    c_Rect = newCapacity;
